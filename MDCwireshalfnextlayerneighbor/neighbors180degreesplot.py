...
neighborchart = np.load('/hpcfs/bes/mlgpu/hoseinkk/MLTracking/bhabha/GNN/MDCwireshalfnextlayerneighbor/share/neighborchart180d.npy', allow_pickle=True)
...
trans_offset_p1 = mtransforms.offset_copy(ax.transData, fig=fige, y = 6, units='dots')
trans_offset_p2 = mtransforms.offset_copy(ax.transData, fig=fige, y = 18, units='dots')
...
plt.title('\n\neast end of the mdc wires\n half of the cells of the next layer are cosidered to be neighbors\n (maxgid_neighbor, mingid_neighbor)\n gid\n karimi@ihep.ac.cn\n\n', fontsize=75)
...
for gid in range(6508):
    plt.text(wiresangles['eastphi'][gid], wiresangles['east_r'][gid], neighborchart[gid][1:3], fontsize = 'x-small', \
        transform=trans_offset_p1, horizontalalignment='center', verticalalignment='bottom')
for gid in range(6508):
    if neighborchart[gid][3] != 0:
        plt.text(wiresangles['eastphi'][gid], wiresangles['east_r'][gid], neighborchart[gid][3:5], fontsize = 'x-small', \
            transform=trans_offset_p2, horizontalalignment='center', verticalalignment='bottom')
plt.savefig('./results/neighborsoneastview180d.png', bbox_inches='tight')
...
plt.savefig('/media/rostam/5C0C24B60C248D58/Users/Rostam/Research/MDCwiresfirstneighbors                    /results/neighborsonwestview.png', bbox_inches='tight')'''